...
st.set_page_config(page_title="Lista 4 - Padrón Completo", page_icon="✌️", layout="wide") # Layout ancho para ver todo
...
if not st.session_state.autenticado:
    ...
    # --- BUSCADOR SIN LÍMITES ---
    st.header(f"Consulta de Padrón - Usuario: {st.session_state.nombre}")

    @st.cache_data
    def cargar_datos():
        for enc in ['latin-1', 'utf-8', 'cp1252']:
            try:
                df = pd.read_csv("datos.csv", encoding=enc, sep=None, engine='python')
                # Mantenemos solo columnas de contacto y ubicación básicas
                cols = [c for c in df.columns if any(x in c.upper() for x in ['DNI', 'MATRICULA', 'NOMBRE', 'APELLIDO', 'DIRECCION', 'CALLE', 'NRO', 'BARRIO'])]
    ...
    if padron is not None:
        busqueda = st.text_input("🔎 BUSCÁ POR CALLE, APELLIDO O DNI (Ej: Sabatini):")
        
        if busqueda:
            registrar_en_google(st.session_state.nombre, st.session_state.localidad, busqueda)
            
            # Buscamos de forma parcial en todo el padrón
            ...
            if not resultados.empty:
                st.success(f"Se encontraron **{len(resultados)}** afiliados con ese término.")
                # Mostramos TODOS los resultados sin paginación corta
                st.dataframe(resultados, use_container_width=True, height=600) 
            else:
                st.warning("No se encontraron coincidencias. Verificá si está bien escrito.")
